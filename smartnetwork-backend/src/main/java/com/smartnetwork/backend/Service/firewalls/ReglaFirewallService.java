...
public class ReglaFirewallService {
    ...
    public ReglaFirewallService(
            ...
            DispositivoReglaFirewallRepository dispositivoReglaFirewallRepository,
    ...
    public void asignarReglaFirewallADispositivos(Long reglaFirewallId, List<Long> dispositivosId, String username) {
        ReglaFirewall reglaFirewall = reglaRepo.findById(reglaFirewallId).orElseThrow(() -> new RuntimeException("ReglaFirewall no encontrada"));
        ...
        for (Long dispositivoId : dispositivosId) {
            Dispositivo dispositivo = dispositivoRepo.findById(dispositivoId).orElseThrow(() -> new RuntimeException("Dispositivo no encontrado"));
            if (!dispositivo.getUsuario().getUsername().equals(username)) {
                throw new RuntimeException("No autorizado");
            }
            boolean yaExiste = dispositivoReglaFirewallRepository.existsByDispositivoIdAndReglaFirewallId(dispositivoId, reglaFirewallId);
            if (yaExiste) {
                continue;
            }
            Map<String, Object> resultado = fortiGateService.crearPolicy(dispositivo, reglaFirewall);
            if (!(Boolean) resultado.get("success")) {
                throw new RuntimeException("Error creando policy en FortiGate");
            }
            ...
            logs.add(logService.crearLog(usu,dispositivo, TipoAccion.CREAR,"Se ha CREADO la Policy "+reglaFirewall.getNombre()));
    ...
    public List<ReglaFirewallDTO> listarPorDispositivo(Long dispositivoId, String username) {
        ...
        return dispositivoReglaFirewallRepository.findByDispositivoId(dispositivoId).stream().map(rel -> toDTO(rel.getReglaFirewall(), dispositivoId)).toList();
    }

    public List<ReglaFirewallDTO> listarPorUsario(String username) {Usuario usuario = usuarioRepository.findByUsername(username).orElseThrow(() -> new RuntimeException("Usuario no encontrado"));
        return reglaRepo.findByUsuarioId(usuario.getId()).stream().map(this::toReglaFirewallDTO).toList();
    }

    @Transactional
    public void eliminarRegla(Long reglaId, List<Long> dispositivosIds, String username) {
        ReglaFirewall regla = reglaRepo.findById(reglaId).orElseThrow(() -> new RuntimeException("Regla no existe"));
        List<DispositivoReglaFirewall> relaciones = dispositivoReglaFirewallRepository.findByReglaFirewallId(reglaId);
        Usuario usu = usuarioRepository.findByUsername(username).orElseThrow(()->new RuntimeException("Usuario no encontrado"));
        List<Log> logs = new ArrayList<Log>();
        ...
        for (DispositivoReglaFirewall relacion : relaciones) {
            ...
            Map<String, Object> resultado = fortiGateService.eliminarReglaFirewall(dispositivo, regla.getNombre());
            if (!(Boolean) resultado.get("success")) {throw new RuntimeException("Error eliminando policy");
            }
            logs.add(logService.crearLog(usu,dispositivo, TipoAccion.ELIMINAR,"Se ha ELIMINADO la Policy "+regla.getNombre()));
    ...
    public ReglaFirewallDTO editarReglaFirewall(Long id, CrearReglaFirewallDTO dto, String username) {
        List<DispositivoReglaFirewall> relaciones = dispositivoReglaFirewallRepository.findByReglaFirewallId(id);
        if (relaciones.isEmpty()) {
            throw new RuntimeException("Relacion no encontrada");
        }
        ReglaFirewall original = relaciones.get(0).getReglaFirewall();
        for (DispositivoReglaFirewall relacion : relaciones) {
        ...
        List<Long> dispositivosEditar = dto.getDispositivosId();
        ...
        if (edicionTotal) {
            ...
            for (DispositivoReglaFirewall relacion : relaciones) {
                Dispositivo dispositivo = relacion.getDispositivo();
                Map<String, Object> resultado = fortiGateService.editarPolicy(dispositivo, original);
                if (!(Boolean) resultado.get("success")) {
                    throw new RuntimeException("Error editando policy");
                }
                logs.add(logService.crearLog(usu,dispositivo, TipoAccion.EDITAR,"Se ha EDITADO la Policy "+original.getNombre()));
            }
            ReglaFirewall saved = reglaRepo.save(original);
            logService.guardarTodos(logs);
            return toReglaFirewallDTO(saved);
        }

        ReglaFirewall nueva = new ReglaFirewall();
        nueva.setNombre(original.getNombre());
        nueva.setUsuario(original.getUsuario());
        nueva.setHabilitada(true);
        aplicarCambios(nueva, dto);
        ReglaFirewall nuevaGuardada = reglaRepo.save(nueva);
        for (DispositivoReglaFirewall relacion : relaciones) {
            Long dispositivoId = relacion.getDispositivo().getId();
            if (!dispositivosEditar.contains(dispositivoId)) {
                continue;
            }
            Dispositivo dispositivo = relacion.getDispositivo();
            fortiGateService.eliminarReglaFirewall(dispositivo, original.getNombre());
            dispositivoReglaFirewallRepository.delete(relacion);
            asignarReglaFirewallADispositivos(nuevaGuardada.getId(), List.of(dispositivoId), username);
            logs.add(logService.crearLog(usu,dispositivo, TipoAccion.EDITAR,"Se ha EDITADO la Policy "+original.getNombre()));
        }
        boolean quedanRelaciones = dispositivoReglaFirewallRepository.existsByReglaFirewallId(original.getId());
        if (!quedanRelaciones) {
            reglaRepo.delete(original);
        }
        logService.guardarTodos(logs);
        return toReglaFirewallDTO(nuevaGuardada);
    }

    private void aplicarCambios(ReglaFirewall regla, CrearReglaFirewallDTO dto) {
        regla.setNombre(dto.getNombre());
        regla.setOrigen(dto.getOrigen());
        regla.setDestino(dto.getDestino());
        regla.setIporigen(dto.getIpOrigen());
    ...
    toReglaFirewallDTO(ReglaFirewall regla) {
        ReglaFirewallDTO dto = new ReglaFirewallDTO();
        dto.setId(regla.getId());
        dto.setNombre(regla.getNombre());
        dto.setOrigen(regla.getOrigen());
